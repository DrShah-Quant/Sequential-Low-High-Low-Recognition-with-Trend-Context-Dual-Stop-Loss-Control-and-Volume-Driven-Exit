...
class AlgoEvent:
    ...
    def can_buy_with_volume(self, current_price, lows, highs, open_price, high_price, low_price):
        ...
        self.available_capital -= volume_to_buy * current_price
        self.buy_with_volume_signals.append((self.trend_labels[-1][0], current_price))

        # Calculate stop loss: max(8% of buy price, second last low)
        ...
        self.evt.consoleLog(f"BUY with volume: {volume_to_buy:.0f} units at price {current_price}, Stop Loss: {stop_loss:.2f}")
    ...
    def on_bulkdatafeed(self, isSync, bd, ab):
        ...
        if self.lastprice is not None:
           ...
           # self.evt.consoleLog("Macro Uptrend Lows:")
           # for price, ts in lows:
           #     self.evt.consoleLog(f"Low: {price}, Time: {ts}")

            highs = self.get_macro_uptrend_highs()
           # self.evt.consoleLog("Macro Uptrend Highs:")
           # for price, ts in highs:
           #     self.evt.consoleLog(f"High: {price}, Time: {ts}")
            ...
            if self.check_buy_pattern(lows, highs):
                self.buyPrice = current_price
                self.buySignal_active = True
                #self.evt.consoleLog(f"Pattern matched: BUY SIGNAL at Price = {current_price}")
